...
def ppi(b):
    ...
    pp, qq = 6, 9 # parameters for geophys prior from Martins and Stedinger p. 740 
    if abs(b) < 0.5:
    ...
    def gpd_lik_internal(params):
        ...
        if abs(k) > kthresh: #gpd
            ...
            if np.any(y <= 0) or alpha <= 0: 
                ...
                ll_gpd = length*sp.log(alpha) + (1-1/k)*np.sum(sp.log(y))
        else: #expon
...
def __fitGEV_mlik_gen__(estim_method, data, LAMBDA=1,  ret=[5, 10, 50, 100, 500], **kwargs):
    ...
    if not is_mlik: init_k = min(max(-0.3, init_k), 0.1)  
